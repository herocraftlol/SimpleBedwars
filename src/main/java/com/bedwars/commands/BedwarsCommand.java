...
public class BedwarsCommand implements CommandExecutor, TabCompleter {
    ...
    private final java.util.Map<CommandSender, Long> pendingDeleteConfirmations = new java.util.HashMap<>();
    private final java.util.Map<CommandSender, String> pendingDeleteArena = new java.util.HashMap<>();
        ...
        switch (action) {
            ...
            case "spec" -> {
                ...
                player.sendMessage(PREFIX + "Spawn des spectateurs défini pour " + arena.getName()
                        + " (une cage invisible a été construite autour).");
            }
    ...
    private boolean handleDeleteArena(CommandSender sender, String[] args) {
        ...
        Long pendingSince = pendingDeleteConfirmations.get(sender);
        String pendingArena = pendingDeleteArena.get(sender);
        boolean confirmed = pendingSince != null && pendingArena != null
                && pendingArena.equalsIgnoreCase(name)
                && (System.currentTimeMillis() - pendingSince) <= 15_000L;

        if (!confirmed) {
            pendingDeleteConfirmations.put(sender, System.currentTimeMillis());
            pendingDeleteArena.put(sender, name);
            sender.sendMessage(PREFIX + ChatColor.RED + "" + ChatColor.BOLD + "ATTENTION: " + ChatColor.RESET
                    + ChatColor.RED + "cette action va supprimer entièrement l'arène " + ChatColor.YELLOW + name
                    + ChatColor.RED + " (configuration, lits, générateurs, etc.), c'est irréversible.");
            sender.sendMessage(PREFIX + ChatColor.YELLOW + "Retapez " + ChatColor.WHITE + "/bd delete " + name
                    + ChatColor.YELLOW + " dans les 15 secondes pour confirmer.");
            return true;
        }

        pendingDeleteConfirmations.remove(sender);
        pendingDeleteArena.remove(sender);
        ...
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("delete") || args[0].equalsIgnoreCase("join")) {
                ...
                options.addAll(List.of("equipe", "pos1", "pos2", "posconfirm", "bed", "spawn",
                        "item", "shop", "spec", "save", "config", "delete"));
            }
