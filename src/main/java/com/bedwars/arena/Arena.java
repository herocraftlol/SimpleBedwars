...
public class Arena {
    ...
    // Spawn des spectateurs (une cage invisible y est automatiquement construite autour)
    ...
    /**
     * Calcule automatiquement le centre de la plateforme de lobby flottante,
     * centrée au-dessus de la zone de jeu (gamePos1/gamePos2), à une hauteur
     * donnée au-dessus du point le plus haut de la map.
     */
    public Location computeLobbyPlatformCenter(int heightAboveMap) {
        if (gamePos1 == null || gamePos2 == null) return null;
        double centerX = (gamePos1.getX() + gamePos2.getX()) / 2.0;
        double centerZ = (gamePos1.getZ() + gamePos2.getZ()) / 2.0;
        double topY = Math.max(gamePos1.getY(), gamePos2.getY()) + heightAboveMap;
        Location center = gamePos1.clone();
        center.setX(centerX);
        center.setZ(centerZ);
        center.setY(topY);
        return center;
