...
public class GuiSlot {
    ...
    public static GuiSlot shopItem(Material material, int price, GeneratorType currency) {
        return shopItem(material, 1, price, currency);
    }

    public static GuiSlot shopItem(Material material, int amount, int price, GeneratorType currency) {
        GuiSlot slot = new GuiSlot();
        slot.kind = Kind.SHOP_ITEM;
        slot.material = material;
        slot.amount = Math.max(1, amount);
        slot.price = price;
